...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="mb-0"><i class="bi bi-arrow-up-circle text-danger"></i> سندات الصرف</h2>
        <div class="d-flex gap-2">
            <button class="btn btn-outline-secondary btn-sm" onclick="exportData()"><i class="bi bi-download"></i> تصدير</button>
            <button class="btn btn-danger" onclick="openAddModal()"><i class="bi bi-plus-lg"></i> سند صرف جديد</button>
        </div>
    </div>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead><tr><th>#</th><th>الرقم</th><th>التاريخ</th><th>الحساب</th><th>المورد</th><th>المبلغ</th><th>طريقة الدفع</th><th>إجراءات</th></tr></thead>
                    <tbody id="rows"></tbody>
                </table>
            </div>
...
<div class="modal fade" id="itemModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title">سند صرف جديد</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            ...
            <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إلغاء</button><button type="button" class="btn btn-danger" onclick="saveItem()" id="saveBtn">صرف</button></div>
        </div>
    </div>
</div>
@include('components.confirmation-modal')
<script>
const API='/api/v1/payment-vouchers';let currentPage=1,lastPage=1,searchTimer;
...
<td>${r.account_name||r.account?.name||'-'}</td><td>${r.supplier_name||r.supplier?.name||'-'}</td>
...
document.getElementById('mobileCards').innerHTML=rows.map(r=>`<div class="mobile-card"><div class="mobile-card-title"><span>${r.voucher_no||'#'+r.id}</span></div><div class="mobile-card-row"><span>المورد</span><span>${r.supplier_name||'-'}</span></div><div class="mobile-card-row"><span>المبلغ</span><span class="fw-bold">${formatMoney(r.amount)}</span></div><div class="d-flex gap-1 mt-2"><button class="btn btn-sm btn-outline-danger" onclick="delItem(${r.id})"><i class="bi bi-trash"></i></button></div></div>`).join('');}
...
async function openAddModal(){document.getElementById('fDate').value=new Date().toISOString().substring(0,10);document.getElementById('fSupplierId').value='';document.getElementById('fSupplierSearch').value='';document.getElementById('fAmount').value='';document.getElementById('fNotes').value='';try{const r=await fetch('/api/v1/chart-of-accounts?per_page=100',{headers:{Accept:'application/json'}});const j=await r.json();document.getElementById('fAccountId').innerHTML='<option value="">اختر الحساب</option>'+(j.data||[]).map(a=>`<option value="${a.id}">${a.name}</option>`).join('');}catch(e){}new bootstrap.Modal(document.getElementById('itemModal')).show();}
...
async function saveItem(){const d={voucher_date:document.getElementById('fDate').value,account_id:document.getElementById('fAccountId').value,supplier_id:document.getElementById('fSupplierId').value,amount:document.getElementById('fAmount').value,payment_method:document.getElementById('fPayMethod').value,notes:document.getElementById('fNotes').value};if(!d.amount)return showToast('تنبيه','المبلغ مطلوب','warning');const btn=document.getElementById('saveBtn');btn.classList.add('btn-loading');btn.disabled=true;try{const o={method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF_TOKEN,Accept:'application/json'},body:JSON.stringify(d)};const r=await fetch(API,o);const j=await r.json();if(j.id||j.success){bootstrap.Modal.getInstance(document.getElementById('itemModal')).hide();loadData();showToast('نجاح','تم إنشاء سند الصرف','success');}else showToast('خطأ',j.message||'حدث خطأ','danger');}catch(e){showToast('خطأ','خطأ في الاتصال','danger');}btn.classList.remove('btn-loading');btn.disabled=false;}
...
async function exportData(){try{const r=await fetch(`${API}?per_page=1000`,{headers:{Accept:'application/json'}});const j=await r.json();exportCsv(j.data||[],'payment-vouchers.csv');}catch(e){showToast('خطأ','خطأ في التصدير','danger');}}
loadData();
</script>
@endsection