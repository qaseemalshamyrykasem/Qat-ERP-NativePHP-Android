...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="mb-0"><i class="bi bi-people text-success"></i> العملاء</h2>
        <div class="d-flex gap-2">
            <button class="btn btn-outline-secondary btn-sm" onclick="exportData()"><i class="bi bi-download"></i> تصدير</button>
            <button class="btn btn-success" onclick="openAddModal()"><i class="bi bi-plus-lg"></i> إضافة عميل</button>
        </div>
    </div>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead><tr><th>#</th><th>الاسم</th><th>الهاتف</th><th>العنوان</th><th>الديون</th><th>المدفوع</th><th>المتبقي</th><th>الحالة</th><th>إجراءات</th></tr></thead>
                    <tbody id="rows"></tbody>
                </table>
            </div>
...
<div class="modal fade" id="itemModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title" id="modalTitle">إضافة عميل</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
...
<script>
const API='/api/v1/customers';let currentPage=1,lastPage=1,searchTimer;
...
<td>${r.id}</td><td class="fw-semibold">${r.name}</td><td><span dir="ltr">${r.phone||'-'}</span></td><td>${r.address||'-'}</td>
<td>${formatMoney(r.total_debt)}</td><td>${formatMoney(r.total_paid)}</td><td><span class="${(r.remaining||0)>0?'text-danger fw-bold':'text-success'}">${formatMoney(r.remaining)}</span></td>
<td>${statusBadge(r.status||'active')}</td><td><div class="btn-group btn-group-sm">
...
</div></td></tr>`).join(''):'<tr><td colspan="9" class="text-center text-muted py-4">لا توجد بيانات</td></tr>';
document.getElementById('mobileCards').innerHTML=rows.map(r=>`<div class="mobile-card"><div class="mobile-card-title"><span>${r.name}</span>${statusBadge(r.status||'active')}</div><div class="mobile-card-row"><span>الهاتف</span><span dir="ltr">${r.phone||'-'}</span></div><div class="mobile-card-row"><span>المتبقي</span><span class="${(r.remaining||0)>0?'text-danger':''}">${formatMoney(r.remaining)}</span></div><div class="d-flex gap-1 mt-2"><button class="btn btn-sm btn-outline-primary" onclick="editItem(${r.id})"><i class="bi bi-pencil"></i></button>${r.phone?`<button class="btn btn-sm btn-outline-success" onclick="waSend('${r.phone}')"><i class="bi bi-whatsapp"></i></button>`:''}<button class="btn btn-sm btn-outline-danger" onclick="delItem(${r.id})"><i class="bi bi-trash"></i></button></div></div>`).join('');}
...
function openAddModal(){document.getElementById('editId').value='';document.getElementById('modalTitle').textContent='إضافة عميل';['fName','fPhone','fAddress','fNotes'].forEach(id=>document.getElementById(id).value='');new bootstrap.Modal(document.getElementById('itemModal')).show();}
async function editItem(id){try{const r=await fetch(`${API}/${id}`,{headers:{Accept:'application/json'}});const d=await r.json();document.getElementById('editId').value=d.id;document.getElementById('modalTitle').textContent='تعديل عميل';document.getElementById('fName').value=d.name;document.getElementById('fPhone').value=d.phone||'';document.getElementById('fAddress').value=d.address||'';document.getElementById('fNotes').value=d.notes||'';new bootstrap.Modal(document.getElementById('itemModal')).show();}catch(e){showToast('خطأ','حدث خطأ','danger');}}
async function saveItem(){const id=document.getElementById('editId').value;const d={name:document.getElementById('fName').value,phone:document.getElementById('fPhone').value,address:document.getElementById('fAddress').value,notes:document.getElementById('fNotes').value};if(!d.name)return showToast('تنبيه','اسم العميل مطلوب','warning');const btn=document.getElementById('saveBtn');btn.classList.add('btn-loading');btn.disabled=true;try{const o={method:id?'PUT':'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF_TOKEN,Accept:'application/json'},body:JSON.stringify(d)};const r=await fetch(id?`${API}/${id}`:API,o);const j=await r.json();if(j.id||j.success){bootstrap.Modal.getInstance(document.getElementById('itemModal')).hide();loadData();showToast('نجاح',id?'تم التعديل':'تم الإضافة','success');}else showToast('خطأ',j.message||'حدث خطأ','danger');}catch(e){showToast('خطأ','خطأ في الاتصال','danger');}btn.classList.remove('btn-loading');btn.disabled=false;}
...
async function exportData(){try{const r=await fetch(`${API}?per_page=1000`,{headers:{Accept:'application/json'}});const j=await r.json();exportCsv(j.data||[],'customers.csv');}catch(e){showToast('خطأ','خطأ في التصدير','danger');}}
